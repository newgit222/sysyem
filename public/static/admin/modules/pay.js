        ...
        i.render({
            ...
            i.on("tool(app-pay-code-list)",
                function (e) {
                    ...
                    }else if ("sysNullRate" === e.event) {
                        //为每个pay_code的渠道赋值权重
                        t(e.tr);


                        layer.confirm('确定要将所有码商费率清0吗', {
                            btn: ['确定', '取消'] //可以无限个按钮
                            ,title:'将所有码商费率清0'
                    ...
                    }  else if ("sysMsRate" === e.event) {
                        //为每个pay_code的渠道赋值权重
                        t(e.tr);


                        layer.confirm('<input id="tz_rate" placeholder="请输入需要调整的费率" lay-verify="number"style="border: 1px solid #e6e6e6;" type="text" class="layui-input" >', {
                            btn: ['上调', '下调'] //可以无限个按钮
                            ,title:'调整码商费率（统一按照百分比计算）'
                        }, function(index, layero){
                            //按钮【按钮一】的回调
                            var tz_rate = t('#tz_rate').val();
                            if (tz_rate == ''){
                                alert('请正确输入调整费率');
                                return false;
                           ...
                           t.ajax({
                                url: "sysMsRate",
                                method: 'POST',
                                data: {tz_rate: tz_rate, pay_code_id: d.id,status:1},
                                success: function (res) {
                                    layer.close(index); //关闭弹层
                                    layer.msg(res.msg, {icon: res.code == 1 ? 1 : 2, time: 1500});
                                }
                            ...
                            t.ajax({
                                url: "sysMsRate",
                                method: 'POST',
                                data: {tz_rate: tz_rate, pay_code_id: d.id,status:2},
                                success: function (res) {
                                    layer.close(index); //关闭弹层
                                    layer.msg(res.msg, {icon: res.code == 1 ? 1 : 2, time: 1500});
                                }
