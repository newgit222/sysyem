...
use think\Db;
use think\Log;
class BingLiLaBiPay extends DaifuPayment
{
    public function pay($params,$dfChannel){
        ...
        Log::error('BingLiLaBiPay Api Data :'.json_encode($data,true));
        ...
        Log::error('BingLiLaBiPay Return Data :'.$result);
...
//        $string_a = substr($string_a,0,strlen($string_a) - 1);
        //签名步骤三：MD5加密
        Log::error('BingLiLaBiPaysign str :'.$string_a . 'key=' . $secret);
...
//        $result = strtoupper($sign);
    ...
    public function notify(){
        ...
        Log::notice("Daifu BingLiLaBiPay Notify post json Data".$input);
        $notifyData = $_POST;
        Log::notice('Daifu BingLiLaBiPay Notify post form Data:' . json_encode($notifyData,true));
...
//        Log::error('Daifu OgDfPay Notify Error Data:' . json_encode($notifyData,true));
    ...
    public function balance($channel){
        ...
        Log::error('BingLiLaBiPaybalancePay balance Api Data :'.json_encode($data,true));
        $headers = array(
            "Content-type: application/json;charset='utf-8'",
            "Accept: application/json",
        );
        ...
        Log::error('BingLiLaBiPaybalancePay balance Return Data :'.$result);
