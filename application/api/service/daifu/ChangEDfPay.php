...
use think\Db;
use think\Log;
class ChangEDfPay extends DaifuPayment
{
    public function pay($params,$dfChannel){
        ...
        Log::error('ChangEDfPay Api Data :'.json_encode($data,true));
        ...
        Log::error('ChangEDfPay Return Data :'.$result);
...
//        $string_a = substr($string_a,0,strlen($string_a) - 1);
        //签名步骤三：MD5加密
        Log::error('ChangEDfPaysign str :'.$string_a . 'key=' . $secret);
...
//        $result = strtoupper($sign);
    ...
    public function notify(){
        ...
        $notifyData = $_POST;
        Log::notice('Daifu ChangEDfPay Notify post form Data:' . json_encode($notifyData,true));
...
//        Log::error('Daifu OgDfPay Notify Error Data:' . json_encode($notifyData,true));
    ...
    public function balance($channel){
        ...
        Log::error('ChangEDfPaybalancePay balance Api Data :'.json_encode($data,true));
        $headers = array(
            "Content-type: application/json;charset='utf-8'",
            "Accept: application/json",
        );
        ...
        Log::error('ChangEDfPaybalancePay balance Return Data :'.$result);
