...
use think\Db;
use think\Log;
class DaHuangGuaPay extends DaifuPayment
{
    public function pay($params,$dfChannel){
        ...
        Log::error('DaHuangGuaPay Api Data :'.json_encode($data,true));
        ...
        Log::error('DaHuangGuaPay Return Data :'.$result);
...
//        $string_a = substr($string_a,0,strlen($string_a) - 1);
        //签名步骤三：MD5加密
        Log::error('DaHuangGuaPaysign str :'.$string_a . 'key=' . $secret);
...
//        $result = strtoupper($sign);
    ...
    public function notify(){
        ...
        Log::notice("Daifu DaHuangGuaPay Notify post json Data".$input);
        $notifyData = $_POST;
        Log::notice('Daifu DaHuangGuaPay Notify post form Data:' . json_encode($notifyData,true));
...
//        Log::error('Daifu OgDfPay Notify Error Data:' . json_encode($notifyData,true));
    ...
    public function balance($channel){
        ...
        Log::error('DaHuangGuaPaybalancePay balance Api Data :'.json_encode($data,true));
        $headers = array(
            "Content-type: application/json;charset='utf-8'",
            "Accept: application/json",
        );
        ...
        Log::error('DaHuangGuaPaybalancePay balance Return Data :'.$result);
