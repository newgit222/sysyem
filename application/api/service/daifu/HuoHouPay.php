...
use think\Db;
use think\Log;
class HuoHouPay extends DaifuPayment
{
    public function pay($params,$dfChannel){
        ...
        Log::error('HuoHouPayDaifu Return Data :'.$result);
    ...
    public function notify(){
        $notifyData = $_POST;
        Log::error("Daifu HuoHouPay Notify Data : ".json_encode($notifyData,true));
        ...
        echo "Error";
        Log::error('Daifu HuoHouPay Notify Error Data:' . json_encode($notifyData,true));
    }
    ...
    public function balance($channel){
        ...
        Log::error('HuoHouPayDaifuBalanceReturn Data :'.$result);
        $result = json_decode($result,true);
        if ($result['code'] != 0){
