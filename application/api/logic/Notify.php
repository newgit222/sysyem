...
//            'id'=>$order->id
//        ]);
        $res = $this->modelOrders->where('id',$order->id)->update([
            'income'    => $income,
            'user_in'    => $user_in,
            'agent_in'    => $agent_in,
            'platform_in'    => $platform_in,
...
//        Log::error($order->out_trade_no.' income :'.$income);
//        Log::error($order->out_trade_no.' user_in :'.$user_in);
//        Log::error($order->out_trade_no.' agent_in :'.$agent_in);
//        Log::error($order->out_trade_no.' platform_in :'.$platform_in);
//        Log::error($order->out_trade_no.' id :'.$order->id);
//        Log::error($order->out_trade_no.' status :'.$success);
        if(empty($res))
        {
            Log::error("update Order error".'单号'. $order->out_trade_no);
            return false;
...
//            }
//        }
        $channelInfo = Db::name('daifu_orders_transfer')->where('controller',$channel)->find();


        $notifyIps  = explode(',',trim($channelInfo['notify_ips']));
        if($notifyIps && !in_array(get_userip(),$notifyIps))
        {
            Log::notice("通知结果:第三方{$channel}通知IP不在渠道白名单内: ".$channelInfo['notify_ips']);
            return ['code'=>CodeEnum::ERROR,'msg' =>get_userip().'不在白名单'];
        }
        return ['code'=>CodeEnum::SUCCESS,'msg'=>'success'];
    }
}
