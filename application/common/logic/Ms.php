...
class Ms extends BaseLogic
{
    ...
    public function register($username, $password, $re_password,$pid=0,$status=1,$level=1,$admin_id=0)
    {
        ...
        if (empty($username) || strlen($username) < 3 || strlen($username) > 21) {
            return ['code' => CodeEnum::ERROR, 'msg' => '用户名必须大于3位小于21位！'];
        }
        ...
        $data['is_daifu'] = 0;
        $cash_pledge = getAdminPayCodeSys('ms_order_min_money',256,$admin_id);
        if (empty($cash_pledge) || $cash_pledge < 0){
            $cash_pledge = 2000;
        }
        $data['cash_pledge'] = $cash_pledge;
    ...
    public function addMs($data,$pid=0,$status=1,$admin_id=0)
    {
        ...
        if (isset($data['pid'])){
            ...
            if ($ms_level >= 10){
                return ['code' => CodeEnum::ERROR, 'msg' => '最多添加7级代理哦!'];
            }
            $level = $ms_level+1;
...
//            }
        ...
        if($admin_id == 0){
            ...
            $add_name= '码商：'.Db::name('ms')->where('userid',$pid)->value('username');
        }else{
            $add_name = '管理员'.Db::name('admin')->where('id',$admin_id)->value('username');
        }
        action_log('添加码商',$add_name.'添加码商：'.$username);
...
//            'ms_id' => $order->gema_userid,
     ...
     * 拉黑ip
     */
    public function add_ms_callback_ip($data)
    {
        $where = [];
        if (!filter_var($data['ip'], FILTER_VALIDATE_IP)) {
            return ['code' => CodeEnum::ERROR, 'msg' => 'ip格式错误'];
        }
        if (is_admin_login() != 1){
            $where['admin_id'] = is_admin_login();
        }
        $ms = $this->modelMs->where($where)->where('userid', $data['msid'] ?? 0)->find();
        if (empty($ms)){
            return ['code' => CodeEnum::ERROR, 'msg' => '码商不存在'];
        }



        $ms_callback_ip = explode(',',$ms['monitor_ip']);
        if (in_array($data['ip'],$ms_callback_ip)){
            return ['code' => CodeEnum::ERROR, 'msg' => '该ip已经在白名单了'];
        }
        array_push($ms_callback_ip,$data['ip']);


//        print_r($ms_callback_ip);die;
        $update = ['monitor_ip'=>implode(',',$ms_callback_ip)];
...
//        }
        ...
        $ret = $this->modelMs->where($where)->where('userid', $data['msid'] ?? 0)->update($update);

        if ($ret === false){
            return ['code' => CodeEnum::ERROR, 'msg' => 'ip加入白名单失败'];
        }else{
            return ['code' => CodeEnum::SUCCESS, 'msg' => 'ip已成功加入白名单'];
        }

    }


    public function getSonMsIds($parentid)
    {

        $list = Db::name("ms")->where(["pid" => $parentid])->field('userid')->select();
        foreach ($list as $key => $value) {
            $this->son_id[] = $value['userid'];
            $this->getSonMsIds($value['userid']);
        }
        return $this->son_id;
    }
