...
//            }
         ...
         //   $jobData['scene'] = 'register';
          //  $this->logicQueue->pushJobDataToQueue('AutoEmailWork', $jobData, 'AutoEmailWork');
            $where =[];
            // 查询 支付编码
            $PayCode = $this->logicPay->getCodeList($where, true, 'create_time desc', false);
            $userCode = [];
            foreach ($PayCode as $k => $v) {
                $userCode[$k]['uid'] = $user;
                $userCode[$k]['co_id'] = $v['id'];
                $userCode[$k]['status'] = 1;
            }

            if (!empty($userCode)) {
                $ret = $this->modelUserPayCode->insertAll($userCode);
            }
...
//        }
        ...
        try {
            ...
            if (isset($data['auth_login_ips'])) {
                ...
                $data['auth_login_ips'] = implode(',', $auth_ips);
            }else{
                $data['auth_login_ips'] = '';
            }


            if (isset($data['auth_ips'])){
...
//                    }
                ...
                $api_auth_ips = implode(',', $data['auth_ips']);
...
//                print_r($data['pao_ms_ids']);die;
            ...
            if (isset($data['daifu_transfer_ids'])) {
                $admin_id = Db::name('user')->where('uid',$data['uid'])->value('admin_id');
                foreach ($data['daifu_transfer_ids'] as $k=>$ip) {
                    $ip = trim($ip);
                    $transfer_admin_id = Db::name('daifu_orders_transfer')->where('id',$ip)->value('admin_id');
                    if (empty($ip)){
                        unset($data['daifu_transfer_ids'][$k]);
                    }
                }
                $data['daifu_transfer_ids'] = array_unique($data['daifu_transfer_ids']);
                $data['daifu_transfer_ids'] = implode(',',$data['daifu_transfer_ids']);

            }else{
                $data['daifu_transfer_ids'] = '';
            }
...
//                $pao_ms_ids = $data['pao_ms_ids'];
                // $data['team_ids'] = explode(',', $data['team_ids']) ? explode(',', $data['team_ids']) : [];

                //验证ip
                $admin_id = Db::name('user')->where('uid',$data['uid'])->value('admin_id');
                foreach ($data['team_ids'] as $k=>$ip) {
                    $ip = trim($ip);
                    if (empty($ip)) {
                        continue;
                    }
                    $ms_admin_id = Db::name('ms')->where('userid',$ip)->value('admin_id');
                    if ($admin_id != $ms_admin_id){
                        unset($data['team_ids'][$k]);
                    }
                }
                $data['team_ids'] = array_unique($data['team_ids']);
//                print_r($data['pao_ms_ids']);die;
                $data['team_ids'] = implode(',',$data['team_ids']);
...
//            print_r($data);die;
     ...
     * @return array
     */
    public function getAgentUserIds($uid)
    {
        $where['puid'] = $uid;
        $subUserIds = $this->where($where)->column('uid');

        $allSubUserIds = $subUserIds;
        foreach ($subUserIds as $subUserId) {
            $allSubUserIds = array_merge($allSubUserIds, $this->getAgentUserIds($subUserId));
        }

        return $allSubUserIds;
    }
}
