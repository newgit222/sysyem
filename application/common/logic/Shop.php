...
class Shop extends BaseLogic
{
    ...
    public function saveShop($data)
    {
        ...
        try{
            ...
            $validate = $this->validateShop->scene(isset($data['id']) ? 'edit' : 'add')->check($data);

            if (!$validate){
                return ['code' => CodeEnum::ERROR, 'msg' => $this->validateShop->getError()];
            }

            $this->modelShop->setInfo($data);
