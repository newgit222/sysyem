...
//        }
        ...
        if (empty($codeInfo)) {
            return ['errorCode' => '400028', 'msg' => 'qu dao bu cun zai ' . $order['channel']];
        }
//        $codeInfo = $codeInfo->toArray();
        ...
        if (empty($newChannels)) {
            return ['errorCode' => '400028', 'msg' => 'jin e bu cun zai '];
        }
        ...
        if (empty($channelsMap)) {
            return ['errorCode' => '400006', 'msg' => 'lu you zhi fu cuo wu [wu ke yong qu dao ]'];
        }
       ...
       // $limitMoney = trim($channel['limit_moneys']) ? array_filter(explode(',', $channel['limit_moneys'])) : [];
        if ($limitMoney) {
            ...
            if (!in_array($order['amount'] * 100, $limitMoney)) {
                return ['errorCode' => '400008', 'msg' => 'ding dan jin e bu yun xu '];
            }
        }
        ...
        if (empty($accountsMap)) {
            return ['errorCode' => '400008', 'msg' => 'lu you zhi fu cuo wu [mei you ke yong de shang jia zhang hu ]'];
        }
        ...
        if (is_null($accountConf)) {
            return ['errorCode' => '400008', 'msg' => 'lu you zhi fu cuo wu [zhi fu zhang hu pei zhi cuo wu ]'];
        }
        //配置合并
...
from cm_pay_channel pc  where pc.status = 1;');
    ...
    public function saveCodeInfo($data)
    {
        ...
        try {
            ...
            if (isset($data['code']) && $data['code']) $data['code'] = trim($data['code']);
