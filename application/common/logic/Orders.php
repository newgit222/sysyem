...
//        $this->modelOrders->join($join);
     ...
     * @return mixed
     */
    public function getOrderList4($where = [], $field = true, $order = 'create_time desc', $paginate = 15)
    {
        $this->modelOrders->alias('a');

        $join = [
            ['user u', 'a.uid = u.uid', 'left'],
            ['admin am', 'u.admin_id = am.id', 'left'],
            ['ewm_order eo', 'a.trade_no = eo.order_no', 'left'],
            ['ms m', 'eo.gema_userid = m.userid', 'left'],
            ['pay_account b', 'a.cnl_id = b.id', 'left'],
            ['pay_channel c', 'b.cnl_id = c.id', 'left'],
            ['pay_code d', 'a.channel = d.code  ', 'left'],
            ['orders_notify e', 'e.order_id = a.id', 'left'],
            ['ewm_pay_code ec', 'eo.code_id = ec.id', 'left'],
        ];
    ...
    public function getOrdersCount4($where = [])
    {

        return $this->modelOrders->alias('a')
            ->where($where)
            ->join('user u','a.uid = u.uid', 'left')
            ->join('admin am', 'u.admin_id = am.id', 'left')
            ->join('ewm_order eo', 'a.trade_no = eo.order_no', 'left')
            ->join('ms m', 'eo.gema_userid = m.userid', 'left')
            ...
            ->join('ewm_pay_code ec', 'eo.code_id = ec.id', 'left')
            ->count();
...
//            'order' => $this->modelEwmOrder->getInfo($whereOrder, "count(a.id) as total,COALESCE(cast(sum(a.order_price) AS decimal(15,2)),0) as fees,COALESCE(cast(sum(if(a.status=1,a.order_price,0)) AS decimal(15,2)),0) as paid,count(if(a.status=0,true,null)) as wait,count(if(a.status=1,true,null)) as success", $order, $paginate = false),
            'order' => ['total'=>'暂停统计','fees'=>'暂停统计' ,'paid' => '暂停统计','wait'=>'暂停统计','success'=>'暂停统计'],
...
//        $this->modelEwmOrder->join($join);
    ...
    function getOrderUserStat($where = [], $field = "a.uid,b.username,count(a.uid) as total_orders,count(if(a.status=2,true,null)) as total_paid_count,COALESCE(sum(amount),0) as total_fee_all,COALESCE(sum(if(a.status=1,amount,0)),0) as total_fee_dis,COALESCE(sum(if(a.status=2,amount,0)),0) as total_fee_paid,COALESCE(sum(if(a.status=2,platform_in,0)),0) as total_plat_fee", $order = 'a.uid asc', $paginate = 15)
    {
        ...
        $this->modelOrders->join  = $join;
        return $this->modelOrders->getList($where, $field, $order, $paginate = false);
//        return $this->modelOrders->join($join)->limit(100)->order($order)->where($where)->field($field)->select();
    }
     ...
     */
    public
    function pushOrderNotifyV2($order_id = '', $server = false)
    {
        //订单
        $order = $this->getOrderInfo(['id' => $order_id]);
        if (!$order) {
...
//                    $hosts = config('orginal_host');
            ...
            Log::notice("后台补发通知订单异步队列回调 notify url " . $order['notify_url'] . "data：" . json_encode($to_sign_data).'返回内容:'.$contents);
            Log::notice("response code: " . $statusCode . " response contents: " . $contents);

            $notifyContent = (new OrdersNotify())->where(['order_id' => $order['id']])->value('content');

            (new OrdersNotify())->where(['order_id' => $order['id']])->update([
...
//                判断放回是否正确
            ...
            $errorReturn = [
                'callback_data' => $contents,
                'params' => json_encode($to_sign_data),
            ];

            return ['code' => CodeEnum::ERROR, 'msg' => '回调发送失败', 'data' => $errorReturn];

        } catch (\GuzzleHttp\Exception\RequestException $e) {
            Log::error('Notify Error:[' . $e->getMessage() . ']');
            $errorReturn = [
                'callback_data' =>$e->getMessage(),
                'params' => json_encode($to_sign_data) ?? '',
            ];
            return ['code' => CodeEnum::ERROR, 'msg' => $e->getMessage(),'data' => $errorReturn];
        }
    }
...
//        print("<info>md5 str:".$str."</info>\n");
    ...
    function createPayOrder($orderData)
    {
        ...
        try {
            ...
            //抛出错误异常
            throw new OrderException([
                'msg'     => "chuang jian ding dan cuo wu ，qing shao hou zai shi." . $e->getMessage(),
                'errCode' => '200001'
            ]);
        }
    }
