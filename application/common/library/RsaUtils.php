...
//            $crypto .= $encryptData;
//        }
//        $encrypted = $this->_encode($crypto, 'base64');
//        return $encrypted;
//    }
//
//    //私钥解密
...
//    {
//        $crypto = '';
//        foreach (str_split($this->_encode($encrypted, "base64"), 256) as $chunk) {
...
//    }
    ...
    public function PublicEncrypt($data, $code = 'base64', $padding = OPENSSL_PKCS1_PADDING){
        ...
        if ($this->_checkPadding($padding, 'en'))
        foreach (str_split($data, 245) as $chunk) {
            openssl_public_encrypt( $chunk, $result, $this->pubKey, $padding);
            $ret .= $result;
        }
        $ret = $this->_encode($ret, $code);
        return $ret;
    ...
    //私钥解密
    public function PrivateDecrypt($data, $code = 'base64', $padding = OPENSSL_PKCS1_PADDING, $rev = false)
    {
        $ret = false;
        ...
        if ($this->_checkPadding($padding, 'de'))
            if ($data !== false) {
                foreach (str_split($data, 256) as $chunk) {
                    openssl_private_decrypt( $chunk, $result, $this->priKey, $padding);
