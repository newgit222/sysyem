...
class Menu extends BaseAdmin
{
     ...
     * @return string
     */
    public function menuToView($menu_list = [], $child = 'child', $replaceUrl = '')
    {

        $payCode = $this->modelPayCode->where('id', 'LT', '255')->field('id,name as name_str,code')->select();
        $payCodeIds = array_column($payCode, 'id');
        $configWhere = [
            'type' => ['in', $payCodeIds],
            'name' => 'channel_show',
            'admin_id' => is_admin_login()
        ];
        $configs = Db::name('config')->where($configWhere)->field('type,value')->select();
        $configs= array_column($configs, 'value', 'type');
        $channel_hide_arr = [];
        foreach ($configs as $key => $config){
            if ($config == 2){
                $channel_hide_arr[] = array_column($payCode, 'name_str', 'id')[$key] . '列表';
                $channel_hide_arr[] = array_column($payCode, 'name_str', 'id')[$key] . '订单';
            }
        }
        ...
        foreach ($menu_list as $menu_info) {
            ...
            if (!empty($menu_info[$child]) &&  !in_array($menu_info['name'], $channel_hide_arr) ) {
                ...
                $menu_view.= "<li data-id='".$menu_info['id']."' class='layui-nav-item'>
                                  ...
                                  ".$this->menuToView($menu_info[$child],  $child, $replaceUrl)."
                                  </dl>
                                </li>";

            } else {
                $url = url($menu_info['url']);
                $url = $replaceUrl ? $replaceUrl.'?v='.$menu_info['id'] : $url;
                ...
                if ($menu_info['pid'] == 0){
                    ...
                    if (!in_array($menu_info['name'], $channel_hide_arr)){
