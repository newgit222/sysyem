...
class Article extends BaseAdmin
{
    ...
    public function editNotice()
    {
        ...
        $this->noticeCommon();
        $admin_id = $this->modelNotice->where(['id' => $this->request->param('id')])->value('admin_id');

        if ($admin_id !== is_admin_login()){
            $this->error('您没有权限编辑该通知');
        }
    ...
    {
        $this->request->isPost() && $this->result($this->logicArticle->saveNotice(array_merge($this->request->post(), ['admin_id' => is_admin_login()])));
