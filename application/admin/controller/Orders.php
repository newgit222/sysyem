...
//        return $this->modelOrders->where($where)->count();
    ...
    public function index()
    {
        ...
        $this->assign('code', $this->logicPay->getCodeList(['status'=>1,'id'=>['lt',255]]));
...
//            $whereCode['admin_id'] = ['eq', is_admin_login()];
        ...
        $this->assign('code', $this->logicPay->getCodeList($whereCode,$field = true, $order = 'create_time desc', 999));
        return $this->fetch();
    }
...
//            = ['eq', $this->request->param('uid')];
        ...
        if (!empty(trim($this->request->param('username')))) {
            if (is_admin_login() != 1){
                $u_aid = Db::name('user')->where('username',trim($this->request->param('username')))->value('admin_id');
                if (is_admin_login() != $u_aid){
                    $this->error('非法请求');
                }
            }
            $where['b.username'] = ['like', $this->request->param('username')];
        }
...
//    }
    ...
    public function subnotify()
    {
        ...
        if (is_admin_login() != 1) {
            $uid = $this->modelOrders->where('id', $this->request->param('order_id'))->value('uid');

            $admin_id = $this->modelUser->where('uid', $uid)->value('admin_id');
    ...
    public function  exportOrder(){
       ...
       // ini_set('memory_limit', '4096M');
        ...
        if (is_admin_login() != 1) {
            $uids = $this->modelUser->where('admin_id', is_admin_login())->column('uid');
            $where['a.uid'] = ['in', $uids];
        } 
