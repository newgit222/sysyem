...
class User extends BaseAdmin
{
    ...
    public function cal()
    {
        $whereCode['status'] = 1;
        $whereCode['id'] = ['LT','255'];
        $this->assign('code', $this->logicPay->getCodeList($whereCode, $field = true, $order = 'create_time desc', false));
    ...
    public function calList()
    {
        ...
        $where['update_time'] = ['between', [$time, $time + 86400]];
        $data = [];
        //订单总金额+完成金额
        if (is_admin_login() != 1){
        ...
        if(!empty($this->request->param('channel'))){
            $orderWhere['channel'] = $this->request->param('channel');
        }

        $orderInfo = $this->logicOrders->getOrderInfo(array_merge($where,$orderWhere), 'COALESCE(sum(`amount`),0) as amount,COALESCE(sum(if(status=2,amount,0)),0) as paid');
    ...
    public function getList()
    {
        ...
        if ($this->request->get('min_amount') ){
            $where['ap.min_amount'] = ['egt', $this->request->get('min_amount')];
        }

        if ($this->request->get('max_amount') ){
            $where['ap.max_amount'] = ['elt', $this->request->get('max_amount')];
        }
...
//        $data = $this->logicUser->getUserList($where, true, 'create_time desc', false);
        ...
        $data = $this->modelUser
            ...
            ->join('api ap', 'ap.uid = a.uid')
            ->field('a.*,m.username as admin_username,ap.min_amount,ap.max_amount')
            ->where($where)
...
//        print_r($this->request->param('id'));die;
       ...
       if( $this->request->isPost()){
           $apiEdit = [
               'min_amount' => $this->request->post('min_amount'),
               'max_amount' => $this->request->post('max_amount'),
               'is_notify_status' => $this->request->post('is_notify_status'),
           ];

           $this->modelApi->where('uid', $this->request->post('uid'))->update($apiEdit);
    ...
    public function profit()
    {
        ...
        if ($this->request->isPost()) {
            ...
            foreach ($data as $key => $item) {
                if (is_admin_login() !=1){
                    $u_aid = Db::name('user')->where('uid',$item['uid'])->value('admin_id');
...
//        $this->assign('is_open', $is_open);
    ...
    public function daifuProfit()
    {
        ...
        if ($this->request->isPost()) {
            $data = $this->request->post('r/a');
            if (is_admin_login() !=1){
                $u_aid = Db::name('user')->where('uid', $data['uid'])->value('admin_id');
                if ($u_aid != is_admin_login()){
                    return  $this->error('非法操作');
                }
            }
