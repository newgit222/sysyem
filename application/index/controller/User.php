...
class User extends Base
{
	...
	$whether_open_daifu =0 ;
        ...
        //下级用户资金信息
        $subWhere = ['uid' => ['in' , $this->logicUser->getAgentUserIds(is_login())] , 'create_time' => ['between time', [$start, $end]]];

        $this->assign('sub_wallet', $this->logicBalance->getBalanceInfo($subWhere , 'sum(enable) as enable,sum(disable) as disable'));

        //当日下级总跑量
        $this->assign('sub_stat', $this->logicOrders->getUserWelcomeStat($subWhere));

        //当日下级代付总额
        $this->assign('sub_daifu_stat', $this->logicDaifuOrders->where(  array_merge(['status' => 2], $subWhere))->sum('amount'));
...
//        } else {
            $this->assign('stat', $this->logicOrders->getUserWelcomeStat($where, is_login()));
        //当月时间
        //当月数据统计
        $this->assign('month', $this->logicOrders->getUserWelcomeStat($where, is_login()));
        //最新订单  当月时间
