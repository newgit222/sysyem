...
class Balance extends Base
{
    ...
    public function index()
    {
        ...
        //总充值上分
        $czsf_total = $this->modelBalanceChange->where(['uid' => is_login(), 'type_reason' => 1])->sum('increase');

        $this->assign('czsf_total', $czsf_total);
...
//            echo 123; die();
    ...
    public function apply()
    {
        ...
        $user_withdraw_google_code_on = getAdminPayCodeSys('user_withdraw_google_code_on', 256, $userInfo['admin_id']);

        if ($this->request->isPost()) {
            //todo  google身份验证暂时关闭
            if($userInfo['is_need_google_verify'] && $user_withdraw_google_code_on == 1){
                //google身份验证
...
//        session('google_secret', $secret);
     ...
     //   $this->assign('google_qr', $ga->getQRCodeGoogleUrl($userInfo['account'], $secret));
        ...
        $this->assign('user_withdraw_google_code_on', $user_withdraw_google_code_on);
